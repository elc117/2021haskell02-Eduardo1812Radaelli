...
verificaTemp :: Float -> Bool
verificaTemp temp = temp > 37.8 

comFebre :: [Float] -> [Float] 
comFebre lsttemp = filter verificaTemp lsttemp

{- 2. Escreva uma função comFebre' :: [Float] -> [Float] que resolva a questão anterior usando lambda. -}

comFebre' :: [Float] -> [Float]
comFebre' lstfloat = filter (\x -> (x > 37.8)) lstfloat
...
itemize lststr = map (\x -> "<li> " ++ x ++ " </ li>") lststr
...
bigCircles numero lstraios = filter ((\x y -> x < (pi * y^2)) numero) lstraios 
...
quarentena pessoa = filter (\(_,temp) -> (verificaTemp temp)) pessoa
...
idadesEm lstanos anoreferencia = map (\x -> anoreferencia - x) lstanos
...
changeNames lstnomes = map (\x -> if head x == 'A' then ("Super " ++ x) else x) lstnomes
...
onlyShorts lststrings = filter (\x -> length x < 5) lststrings